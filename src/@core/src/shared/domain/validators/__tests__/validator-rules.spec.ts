...
const applyRules = ({ value, property, rule, params = [] }: Omit<ExceptedRule, 'error'>) => {
...
function assertIsInvalid({ error, ...rest }: ExceptedRule) {
    expect(() => applyRules(rest)).toThrow(error);
}

function assertIsValid({ error, ...rest }: ExceptedRule) {
    expect(() => applyRules(rest)).not.toThrow(error);
}
describe('ValidatorRules Unit Tests', () => {
    ...
    test('maxLength validation rule', () => {
        let arrange: Values[] = [{ value: 'aaaaaaaaaaa', property: 'field' }];
    ...
    it('should combine two or more validation rules and throw validation error', () => {
        ...
        expect(() => validator.required().string()).toThrow(new ValidationError('field is required'));
        ...
        expect(() => validator.required().boolean()).toThrow(new ValidationError('field is required'));

        validator = ValidatorRules.values(5, 'field');
        expect(() => validator.required().boolean()).toThrow(new ValidationError('field must be a boolean'));
    });
